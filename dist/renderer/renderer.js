...
function resolveConfig(spec) {
  ...
  if (!rest || !conf)
    return conf || rest;
  let conc2 = (a, b) => (a || none2).concat(b || none2);
...
var liveMarkers = ViewPlugin.fromClass(
  class {
    ...
    build(view) {
      ...
      marks2.sort((a, b) => a.from - b.from);
      return Decoration.set(marks2.map((m) => m.deco.range(m.from, m.to)));
    }
  },
  { decorations: (v) => v.decorations }
);
...
async function save() {
  ...
  if (!current || readOnly2() || saving) return;
...
var editor = createEditor(editorEl, {
