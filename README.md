...
Quire talks to [applenotes](https://github.com/gig3m/applenotes), a daemon that
runs on a Mac with SIP disabled and exposes your notes over HTTP on a tailnet.
Quire is the desktop half: it reads, searches and edits those notes from a Linux
...
Quire reads the same configuration the `notes` CLI uses, so a machine already
...
what a rewrite would flatten and what it would destroy, and Quire honours both:

- A note carrying **attachments or checklists** opens read-only, with a banner
  saying why. Saving it would delete that content, so Quire does not offer to.
